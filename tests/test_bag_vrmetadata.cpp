...
TEST_CASE("test vr metadata create open", "[vrmetadata][create][open]")
{
    TestUtils::RandomFileGuard tmpBagFile;

    constexpr uint32_t kExpectedMinDimX = 1;
    constexpr uint32_t kExpectedMinDimY = 2;
    constexpr uint32_t kExpectedMaxDimX = 100;
    constexpr uint32_t kExpectedMaxDimY = 200;
    constexpr float kExpectedMinResX = 10.1f;
    constexpr float kExpectedMinResY = 20.2f;
    constexpr float kExpectedMaxResX = 111.1f;
    constexpr float kExpectedMaxResY = 222.2f;
    ...
    {  // test create
        ...
        auto cpVrMetadata = constDataset->getVRMetadata();
        REQUIRE(cpVrMetadata);
        ...
        pVrMetadataDescriptor->setMaxDimensions(kExpectedMaxDimX, kExpectedMaxDimY);
        pVrMetadataDescriptor->setMinResolution(kExpectedMinResX, kExpectedMinResY);
        pVrMetadataDescriptor->setMaxResolution(kExpectedMaxResX, kExpectedMaxResY);

        // Read the attributes back.
        auto dims = pVrMetadataDescriptor->getMinDimensions();
        CHECK(std::tuple<uint32_t, uint32_t>(kExpectedMinDimX, kExpectedMinDimY) == dims);

        dims = pVrMetadataDescriptor->getMaxDimensions();
        CHECK(std::tuple<uint32_t, uint32_t>(kExpectedMaxDimX, kExpectedMaxDimY) == dims);

        auto res = pVrMetadataDescriptor->getMinResolution();
        CHECK(std::tuple<float, float>(kExpectedMinResX, kExpectedMinResY) == res);

        res = pVrMetadataDescriptor->getMaxResolution();
        CHECK(std::tuple<float, float>(kExpectedMaxResX, kExpectedMaxResY) == res);
...
TEST_CASE("test vr metadata write read", "[vrmetadata][write][read]")
{
