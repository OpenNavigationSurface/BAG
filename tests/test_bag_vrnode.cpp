...
TEST_CASE("test vr node create open", "[vrnode][create][open]")
{
    TestUtils::RandomFileGuard tmpBagFile;

    constexpr float kExpectedMinHypStrength = 10.1f;
    constexpr float kExpectedMaxHypStrength = 20.2f;
    constexpr uint32_t kExpectedMinNumHypotheses = 100;
    constexpr uint32_t kExpectedMaxNumHypotheses = 200;
    constexpr uint32_t kExpectedMinNSamples = 1000;
    constexpr uint32_t kExpectedMaxNSamples = 2000;
    ...
    {  // test create
        ...
        pVrNodeDescriptor->setMinMaxNumHypotheses(kExpectedMinNumHypotheses,
            kExpectedMaxNumHypotheses);
        pVrNodeDescriptor->setMinMaxNSamples(kExpectedMinNSamples,
            kExpectedMaxNSamples);

        // Read the attributes back.
        const auto minMaxHypStrength = pVrNodeDescriptor->getMinMaxHypStrength();
        CHECK(minMaxHypStrength == std::tuple<float, float>(
            kExpectedMinHypStrength, kExpectedMaxHypStrength));
        ...
        const auto minMaxNSamples = pVrNodeDescriptor->getMinMaxNSamples();
        CHECK(minMaxNSamples == std::tuple<uint32_t, uint32_t>(
...
TEST_CASE("test vr node write read", "[vrnode][write][read]")
{
    ...
    constexpr uint32_t kRowStart = 0;  // not used
    constexpr uint32_t kColumnStart = 0;
    constexpr uint32_t kRowEnd = 0;  // not used
    constexpr uint32_t kColumnEnd = 0;

    REQUIRE_NOTHROW(pVrNode->write(kRowStart, kColumnStart, kRowEnd,
        kColumnEnd, buffer));
    ...
    CHECK(res->num_hypotheses == kExpectedItem0.num_hypotheses);
    UNSCOPED_INFO("Check the expected value of BagVRNodeItem::n_samples.");
    CHECK(res->n_samples == kExpectedItem0.n_samples);
}
