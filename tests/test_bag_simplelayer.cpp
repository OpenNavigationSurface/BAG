...
TEST_CASE("test simple layer write, optional layer, nominal_elevation", "[simplelayer][write][optional_layer][nominal_elevation]")
{
    const TestUtils::RandomFileGuard tmpFileName;

    constexpr float kFloatValue = 564.231f;
    ...
    // Create the dataset.
    {
        ...
        UNSCOPED_INFO("Create Nominal_Elevation layer");
        auto& nominalElevationLayer = pDataset->createSimpleLayer(
                Nominal_Elevation, chunkSize, compressionLevel);

        // Write the data.
        std::array<float, kGridSize> nominallElevation{};

        for (uint32_t row=0; row<kGridSize; ++row)
        {
            for (uint32_t column=0; column<kGridSize; ++column) {
                nominallElevation[column] = kFloatValue;
            }

            auto buffer = reinterpret_cast<uint8_t*>(nominallElevation.data());
    ...
    // Open the dataset and read what was written.
    {
        ...
        std::array<float, kGridSize> kExpectedBuffer;
        kExpectedBuffer.fill(kFloatValue);
