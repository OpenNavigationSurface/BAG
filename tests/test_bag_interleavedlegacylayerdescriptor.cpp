...
using BAG::Layer;
using BAG::Metadata;
using BAG::InterleavedLegacyLayer;
using BAG::InterleavedLegacyLayerDescriptor;
...
TEST_CASE("test interleaved legacy layer descriptor creation",
    "[interleavedlegacylayerdescriptor][create]")
{
    ...
    UNSCOPED_INFO("Check that creating an interleaved legacy layer descriptor does not throw.");
    auto pDescriptor = InterleavedLegacyLayerDescriptor::create(*pDataset,
        Hypothesis_Strength, NODE);
    REQUIRE(pDescriptor);
}

//  GroupType getGroupType() const noexcept;
TEST_CASE("test interleaved legacy layer descriptor get group type",
    "[interleavedlegacylayerdescriptor][getGroupType]")
{
    TestUtils::RandomFileGuard tmpBagFile;
...

    {
        ...
        CHECK(pDescriptor->getGroupType() == kExpectedGroup);
    }
    {
    ...
    }
    {
        ...
        REQUIRE_THROWS(InterleavedLegacyLayerDescriptor::create(*pDataset,
            Hypothesis_Strength, ELEVATION));
    }
}

//  uint8_t getElementSize() const noexcept override;
TEST_CASE("test interleaved legacy layer descriptor get/set element size",
    "[interleavedlegacylayerdescriptor][getElementSize][setElementSize]")
{
    TestUtils::RandomFileGuard tmpBagFile;
