...
u8 *excert_read_xml_seckey(char *filename, u8 *phrase, ExcertErr *errcode)
{
	...
	sec_key_clen = (u32)strlen(cert->key.key);
	if ((sec_key_clen %2) == 1) {
...
static u8 *excert_ext_to_bytes(FILE *ip, u32 *nbytes)
{
	...
	fgets(buffer, STRING_BREAK_LENGTH+10, ip);
	buffer[(buf_len = (u32)strlen(buffer))-1] = '\0'; --buf_len; /* chomp */
	sscanf(buffer, "%04X", &ip_len);
	...
	*nbytes = 0;
	while (!feof(ip) && !found) {
		...
		*nbytes += buf_len;
		if (fgets(buffer, STRING_BREAK_LENGTH+10, ip) == NULL) {
			if (ferror(ip)) {
...
ExcertErr excert_put_object(FILE *op, u8 *obj, ExcertObject obtype)
{
	...
	switch(obtype) {
		case EXCERT_PUBLIC_KEY:
			recog_string = PUBLIC_KEY_STRING;
			op_string = ons_key_to_ascii(obj);
			break;
		case EXCERT_SECRET_KEY:
			recog_string = SECRET_KEY_STRING;
			op_string = ons_key_to_ascii(obj);
			break;
		case EXCERT_SIGNATURE:
			recog_string = SIGNATURE_KEY_STRING;
...
u8 *excert_hash_certificate(char *cert, u32 *nbytes)
{
	...
	while (!feof(ip)) {
		fgets(buffer, 1024, ip);
		if (strncmp(buffer, "signature {", 11) == 0) {
...
ExcertErr excert_sign_certificate(char *cert, char *auth, char *phrase)
{
	...
	if ((sec_key = excert_get_hasp_key(phrase, &rc)) == NULL) {
...
u8 *excert_get_public_key(char *cert)
{
	...
	if ((key = excert_get_object(ip, EXCERT_PUBLIC_KEY)) == NULL) {
 ...
 * Inputs:	doc		Pointer to the start of the XML document
 *			node	Pointer to the child node list of the <user> node
...
static Bool excert_parse_user(xmlDocPtr doc, xmlNodePtr node, xcrtUserInfo *user)
{
	char	*tmp;
	...
	while (node != NULL) {
		if (xmlStrcmp(node->name, (const xmlChar *)"name") == 0) {
			...
			xmlFree(tmp);
		} else if (xmlStrcmp(node->name, (const xmlChar *)"organization") == 0) {
			tmp = xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
			user->organization = strdup(tmp);
			xmlFree(tmp);
		} else if (xmlStrcmp(node->name, (const xmlChar *)"idnum") == 0) {
			tmp = xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
			user->idnum = atoi(tmp);
			xmlFree(tmp);
		} else if (xmlStrcmp(node->name, (const xmlChar *)"keysource") == 0) {
			tmp = xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
			user->keysource = strdup(tmp);
			xmlFree(tmp);
		}
		node = node->next;
	}
 ...
 * Inputs:	doc		Pointer to the XML doc structure in memory
 *			node	Pointer to the XML child node of the <signature> node
...
static Bool excert_parse_signature(xmlDocPtr doc, xmlNodePtr node, xcrtSigInfo *sig)
{
	char		*signature, *tmp;
	OnsCryptErr	rc;

	while (node != NULL) {
		if (xmlStrcmp(node->name, (const xmlChar *)"sigstream") == 0) {
			if (xmlStrcmp((sig->algorithm = xmlGetProp(node, "algorithm")), (const xmlChar*)"openns") != 0 ||
				xmlStrcmp((sig->version = xmlGetProp(node, "version")), (const xmlChar*)"1.0") != 0 ||
				xmlStrcmp(xmlGetProp(node, "encoding"), (const xmlChar*)"hex") != 0) {
				fprintf(stderr, "%s: error: type structure for <sigstream> is invalid.\n", modname);
#ifdef __DEBUG__
			...
			tmp = strdup(sig->algorithm); xmlFree(sig->algorithm); sig->algorithm = tmp;
			tmp = strdup(sig->version); xmlFree(sig->version); sig->version = tmp;
			/* Parse the signature stream, extracting the ASCII characters into a binary internal stream */
			if ((signature = xmlNodeGetContent(node)) == NULL) {
			...
			if (!excert_parse_user(doc, node->xmlChildrenNode, &sig->signer)) {
 ...
 * Inputs:	doc		Pointer to the XML doc structure in memory
 *			node	Pointer to the XML node of the <key>
...
static Bool excert_parse_key(xmlDocPtr doc, xmlNodePtr node, xcrtKeyInfo *key)
{
	xmlChar		*type, *enc;
	char		*keystring, *tmp;
	OnsCryptErr	rc;
...
xcrtCertificate *excert_read_xml_certificate(char *name, ExcertErr *rc)
{
	...
	if ((cur = xmlDocGetRootElement(doc)) == NULL) {
		fprintf(stderr, "%s: error: failed to get root node for \"%s\".\n", modname, name);
		*rc = EXCERT_BAD_FILE;
		return(NULL);
	}
	while (cur != NULL) {
...
#ifdef __DEBUG__
			printf("%s: debug: top: Found entity wrapper node ...\n", modname);
#endif
			child = cur->xmlChildrenNode;
			while (child != NULL) {
				if (xmlStrcmp(child->name, (const xmlChar *)"user") == 0) {
#ifdef __DEBUG__
					printf("%s: debug: child: Found user node ...\n", modname);
#endif
					if (!excert_parse_user(doc, child->xmlChildrenNode, &cert->user)) {
...
#endif
				} else if (xmlStrcmp(child->name, (const xmlChar *)"key") == 0) {
#ifdef __DEBUG__
					printf("%s: debug: child: Found key ...\n", modname);
#endif
					if (!excert_parse_key(doc, child, &cert->key)) {
						fprintf(stderr, "%s: error: failed to parse key information from \"%s\".\n", modname, name);
						*rc = EXCERT_BAD_FILE;
						excert_release_xml_certificate(cert);
						return(NULL);
					}
				} else if (xmlStrcmp(child->name, (const xmlChar *)"signature") == 0) {
#ifdef __DEBUG__
					printf("%s: debug: child: Found signature ...\n", modname);
#endif
					if (!excert_parse_signature(doc, child->xmlChildrenNode, &cert->signature)) {
